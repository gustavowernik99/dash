...
#criar fuções de carregamento de dados
    #cotações Itau - ITUB4 - 2010 a 2024
@st.cache_data
def carregar_dados (empresas) :
    texto_tickers = " ".join(empresas)
    dados_acao = yf.Tickers(texto_tickers)
    cotacoes_acao = dados_acao.history(period="1d", start="2010-01-01",end="2024-09-01")
...
if performance_ativo > 0:
    #:cor[texto]
    texto_performance_carteira = f"performance da carteira com todos os ativos: :top[{performance_carteira:.1%}]"
elif performance_ativo < 0:
    texto_performance_carteira = f"performance da carteira com todos os ativos: :paia[{performance_carteira:.1%}]"
else:
